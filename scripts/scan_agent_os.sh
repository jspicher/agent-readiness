...
find . -maxdepth 4 -name 'sandbox.*' -o -name 'agent-runner.*' 2>/dev/null | grep -v node_modules | head -5
grep -RIl --include='*.yml' --include='*.yaml' 'egress\|network.policy\|allowed.hosts' . 2>/dev/null | grep -v node_modules | head -5
...
echo ""
echo "-- SBOM presence --"
find . -maxdepth 3 -name 'sbom.json' -o -name 'sbom.xml' -o -name 'cyclonedx.json' \
  -o -name 'spdx.json' -o -name 'bom.json' 2>/dev/null | head -10
grep -RIl --include='*.yml' --include='*.yaml' 'syft\|cyclonedx\|spdx-tools\|sbom-action' .github/ 2>/dev/null | head -5

echo ""
echo "-- Provenance attestations / signing --"
grep -RIl --include='*.yml' --include='*.yaml' 'cosign\|sigstore\|attest-build-provenance\|in-toto\|slsa-' .github/ 2>/dev/null | head -5
find . -maxdepth 3 -name '*.intoto.jsonl' -o -name 'provenance.*' 2>/dev/null | head -5

echo ""
echo "-- Agent audit trail / run IDs --"
find . -maxdepth 3 -name 'agent-runs' -type d 2>/dev/null | head -3
find . -maxdepth 4 -name '.scratch' -type d 2>/dev/null | head -3
grep -RIl --include='*.md' -E 'run.id|session.id|agent.run' AGENTS.md CLAUDE.md 2>/dev/null | head -5
...
grep -RIl --include='*.yml' --include='*.yaml' 'eval\|golden.test' .github/workflows/ 2>/dev/null | head -5

echo ""
echo "-- Kill-switch infrastructure --"
grep -RIn --include='*.ts' --include='*.js' --include='*.py' --include='*.go' \
  -E '(disable|halt|kill)[-_]?agent|agent[-_]?(disabled|paused)' . 2>/dev/null | grep -v node_modules | head -5
find . -maxdepth 3 -name 'disable-agent*' -o -name 'pause-agent*' 2>/dev/null | head -5

echo ""
echo "-- Human escalation path --"
find . -maxdepth 3 -iname 'ESCALATE*' -o -iname 'HANDOFF*' 2>/dev/null | head -5
grep -RIl --include='*.md' -i 'escalate\|human.handoff\|when.to.escalate' AGENTS.md CONTRIBUTING.md docs/ 2>/dev/null | head -5
...
grep -RIn --include='*.ts' --include='*.js' --include='*.py' \
  -E 'token.usage|tokens.used|input.tokens|output.tokens|agent.cost|cost.per.run' . 2>/dev/null | grep -v node_modules | head -5
find . -maxdepth 4 -name '*cost*log*' 2>/dev/null | grep -v node_modules | head -5
